...
// High Order Low Pass FIR Filter - AVX512
...
// function to perform FIR filtration with AVX512 support
    ...
    int signalLength,const float coefficients[], int order)
{
    ...
    {
        __m512d sumL = _mm512_setzero_ps();
        ...
        {
            // loads coefficients
            __m512d coeffs = _mm512_loadu_ps(&coefficients[j]);
        ...
        {
            tempL += coefficients[j] * static_cast<double>(inputL[i - j]);
        }
    ...
    for (int i = signalLength - 1; i >= 0; i--)
    {
        __m512d sumR = _mm512_setzero_ps();
        ...
        // Remainder loop
        for (; j < std::min(order, i + 1); j++) {
            tempR += coefficients[j] * static_cast<double>(inputR[i - j]);
        }
...
int main()
{
    cout << endl << "Running fir_avx ..." << endl << endl;
